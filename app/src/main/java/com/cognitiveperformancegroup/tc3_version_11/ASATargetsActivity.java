...
public class ASATargetsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_asatargets);
        ...
        login.setText(login.getText() + domain);
        ...
        try {
            ...
            Toast.makeText(getApplicationContext(), "number of targets: " + numTargets, Toast.LENGTH_LONG).show();
            ...
            for (int i = 0; i < numTargets+1; i++) {
                ...
                editText[index].setGravity(Gravity.START);
                                ...
                                if (checkButtons[index].getBackground().getConstantState() == getDrawable(R.drawable.observed_checkmark_green).getConstantState()) {
                                    ...
                                    comments.setPositiveButton("Save", new DialogInterface.OnClickListener() {
                                        ...
                                        public void onClick(DialogInterface dialog, int which) {
                                            ...
                                            comment = editText[index].getText().toString();
                                           // writeOutputExcel();
                                            ...
                                            if (editText[index].getText().toString().equals("")) {
                                                ...
                                                comment = editButtons[index].getText().toString();
                                            }
                ...
                saveAndExit.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        AlertDialog.Builder instructions = new AlertDialog.Builder(ASATargetsActivity.this, R.style.TC3AlertDialog);
                        final TextView instructionsText = new TextView(ASATargetsActivity.this);

                        instructionsText.setWidth(600);
                        instructionsText.setPadding(20, 20, 20, 20);
                        instructions.setView(instructionsText);
                        instructions.setTitle("Exit Confirmation");
                        instructionsText.setGravity(Gravity.TOP | Gravity.LEFT);
                        instructionsText.setText("Would you like to Save and Exit? Your current data progress will be saved.");
                ...
                File inputWorkbook = new File(/*File Path*/);
                if(inputWorkbook.exists()){
                    ...
                    try {
                        ...
                        Sheet s = workbook.getSheet(sheetNum);

                        for (int m = 0; m < s.getRows(); m++) {

                            Cell cell = s.getCell(0, i);
                            String content = cell.getContents();

                            if (content == group) {

                                for (int n = 1; n < s.getColumns(); n++) {

                                    cell = s.getCell(n, i);

                                    if (cell.getContents().equalsIgnoreCase("1")) {

                                        checkButtons[n-1].setBackground(getDrawable(R.drawable.observed_checkmark_green));

                                    }

                                    editText[n-1].setText(cell.getCellFeatures().getComment());
    ...
    public void writeOutputExcel() {
        ...
        try {
            ...
            if (scenario.equalsIgnoreCase("B1")) {
            ...
            else if (scenario.equalsIgnoreCase("B2")) {
                if(workbook.getSheet("ASA-B2") == null) {
                    sheet = workbook.createSheet("ASA-B2", 1);
                }else{

                    sheet = workbook.getSheet("ASA-B2");

                }
            }
            else if (scenario.equalsIgnoreCase("M1")) {
                if(workbook.getSheet("ASA-M1") == null) {
                    sheet = workbook.createSheet("ASA-M1", 2);
                }else{

                    sheet = workbook.getSheet("ASA-M1");

                }
            }
            else if (scenario.equalsIgnoreCase("M2")) {
                if(workbook.getSheet("ASA-M2") == null) {
                    sheet = workbook.createSheet("ASA-M2", 3);
                }else{

                    sheet = workbook.getSheet("ASA-M2");

                }
            }
            else {
                if(workbook.getSheet("ASA-M3") == null) {
                    sheet = workbook.createSheet("ASA-M3", 4);
                }else{

                    sheet = workbook.getSheet("ASA-M3");
            ...
            try {
                ...
                if (green) {
                    label1 = new Label(currentColumn+1, num, "1");
                }
                else {
                    label1 = new Label(currentColumn+1, num, "0");
                }
